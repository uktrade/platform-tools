...
@freeze_time("2025-01-16 13:00:00")
def test_generate_environment_pipeline_config_creates_file(
    ...
    template_provider.generate_environment_pipeline_config(
        platform_config_for_env_pipelines,
        platform_helper_version="14",
        deploy_repository="uktrade/my-app-deploy",
    )
    ...
    assert base_path == str(Path("terraform/environment-pipelines").absolute())
    assert file_path == "main.tf.json"
    assert overwrite
    ...
    assert local["platform_config"] == '${yamldecode(file("../../platform-config.yml"))}'
    assert local["application"] == '${local.platform_config["application"]}'
    assert local["all_pipelines"] == '${local.platform_config["environment_pipelines"]}'
    assert local["environment_config"] == '${local.platform_config["environments"]}'
    ...
    s3_backend = terraform["backend"]["s3"]
    assert s3_backend["bucket"] == "terraform-platform-state-non-prod-acc"
    assert s3_backend["key"] == "tfstate/application/my-app-pipelines.tfstate"
    assert s3_backend["region"] == "eu-west-2"
    assert s3_backend["encrypt"] is True
    assert s3_backend["kms_key_id"] == "alias/terraform-platform-state-s3-key-non-prod-acc"
    assert s3_backend["dynamodb_table"] == "terraform-platform-lockdb-non-prod-acc"
    ...
    module = json_content["module"]["environment-pipelines"]
    assert (
        module["source"]
        == f"git::https://github.com/uktrade/platform-tools.git//terraform/environment-pipelines?depth=1&ref=14"
    )
    assert module["for_each"] == "${local.all_pipelines}"
    assert module["application"] == "${local.application}"
    assert module["pipeline_name"] == "${each.key}"
    assert module["deploy_repository"] == "uktrade/my-app-deploy"
    assert module["deploy_repository_branch"] == "${each.value.branch}"
    assert module["environments"] == "${each.value.environments}"
    assert module["slack_channel"] == "${each.value.slack_channel}"
    assert module["trigger_on_push"] == "${each.value.trigger_on_push}"
    assert module["env_config"] == "${local.environment_config}"
