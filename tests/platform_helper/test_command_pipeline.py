...
def test_generate_terraform_environment_pipeline_manifest_as_part_of_pipeline_generate(
    git_remote, get_aws_command_or_abort, mock_aws_session, mock_is_terraform, fakefs
):
    mock_codestar_connections_boto_client(mock_aws_session, ["test-app"])
    setup_fixtures(
    ...
    result = CliRunner().invoke(
        generate, args=["--terraform-platform-modules-version", "feature_branch"]
    )
    print(f"OUTPUT CONTENTS: {result.output}")
    print(f"RETURN CODE: {result.exit_code}")
    expected_files_dir = Path("terraform/environment-pipelines/platform-sandbox-test/main.tf")
    ...
    assert 'profile                  = "platform-sandbox-test"' in content
    assert (
        f"git::https://github.com/uktrade/terraform-platform-modules.git//environment-pipelines?depth=1&ref=feature_branch"
        in content
    )
    assert 'application   = "test-app"' in content

    expected_prod_files_dir = Path("terraform/environment-pipelines/platform-prod-test/main.tf")
    assert expected_prod_files_dir.exists()
    content_prod = expected_prod_files_dir.read_text()
    assert "# WARNING: This is an autogenerated file, not for manual editing." in content_prod
    assert "# Generated by platform-helper v0.1-TEST / 2024-10-28 12:00:00" in content_prod
    assert 'profile                  = "platform-prod-test"' in content_prod
    assert (
        f"git::https://github.com/uktrade/terraform-platform-modules.git//environment-pipelines?depth=1&ref=feature_branch"
        in content_prod
    )
    assert 'application   = "test-app"' in content_prod
