...
from commands.copilot_cli import copilot as cli, WAF_ACL_ARN_KEY
...
class TestApplyWAF:
    ...
    def test_require_copilot_dir(self, fakefs):
        ...
        assert result.output == "Cannot find copilot directory. Run this command in the root of the deployment repository.\n"
    ...
    def test_invalid_waf_arn(self, arn, fakefs):
        ...
        assert result.exit_code == 1
        assert result.output == "Cannot add WAF CFN templates: Set a valid `waf-acl-arn` in each ./copilot/environments/*/manifest.yml file\n"

    def test_success(self, fakefs):

        fakefs.create_file("./copilot/environments/development/manifest.yml", contents=f"{WAF_ACL_ARN_KEY}: arn:aws:wafv2:development-dummy-arn")
        fakefs.create_file("./copilot/environments/staging/manifest.yml", contents=f"{WAF_ACL_ARN_KEY}: arn:aws:wafv2:staging-dummy-arn")
        fakefs.create_file("./copilot/environments/production/manifest.yml", contents=f"{WAF_ACL_ARN_KEY}: arn:aws:wafv2:production-dummy-arn")
