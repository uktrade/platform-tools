...
from commands.codebuild_cli import AWS_REGION, codedeploy, create_codedeploy_role, slackcreds
...
def test_check_service_role_does_not_exist(capfd, aws_credentials):
    """Test that check_service_role outputs expects error method when codebuild role does not exist."""
...
def test_check_service_role_returns_role_arn():
    """Test that check_service_role returns an arn string containing account number and role name."""
...
def test_update_parameter():
    """Test that update_paramater creates a parameter with expected Name and Value values where none exists."""
...
def test_check_git_url_valid(url, expected):
    """Test that check_git_url returns unmodified url string when url is valid."""
...
def test_create_codedeploy_role_returns_200(alias_session):
    ...
    response = alias_session.client("iam", region_name=AWS_REGION).get_role(RoleName="ci-CodeBuild-role")["ResponseMetadata"]
    policy = alias_session.client("iam", region_name=AWS_REGION).list_attached_role_policies(RoleName="ci-CodeBuild-role")["AttachedPolicies"][0]
...
def test_create_codedeploy_role_policy_already_exists(alias_session):
    ...
    versions = alias_session.client("iam").list_policy_versions(PolicyArn=f"arn:aws:iam::{account_id}:policy/ci-CodeBuild-policy")["Versions"]
...
def test_create_codedeploy_role_limit_exceeded_exception(alias_session):
    ...
    assert "You have hit the limit of max managed policies, please delete an existing version and try again" in result.output
...
def test_slackcreds(alias_session):
    runner = CliRunner()
    result = runner.invoke(slackcreds, ["--workspace", "workspace", "--channel", "channel", "--token", "token", "--project-profile", "foo"], input="y")
