...
""")
    ...
    assert expect_jsonschema_validation_error(storage) == \
        "'not-valid' is not one of ['rds-pg', 'aurora-pg', 'redis', 'opensearch', 's3', 's3-policy']"
...
""")
    ...
    assert expect_jsonschema_validation_error(storage) == "Additional properties are not allowed ('an-extra-key' was unexpected)"
...
def test_schema_postgres_plans_match_available_plans():
    assert set(schema["definitions"]["rds-pg-plans"]["enum"]) == set(plans["rds-pg"].keys())
