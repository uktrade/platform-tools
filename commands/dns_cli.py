...
def wait_for_certificate_validation(
    acm_client, certificate_arn, sleep_time=5, timeout=600
):
    print("waiting for cert...")
    status = acm_client.describe_certificate(CertificateArn=certificate_arn)[
        "Certificate"
    ]["Status"]
    elapsed_time = 0
    while status == "PENDING_VALIDATION":
        ...
        print(
            f"{certificate_arn}: Waiting {sleep_time}s for validation, {elapsed_time}s elapsed..."
        )
...
def create_cert(client, domain_client, domain, base_len):
    ...
    while (
        ...
        or response["Certificate"]["DomainValidationOptions"][0].get("ResourceRecord")
        is None
    ):
    ...
    cert_record = response["Certificate"]["DomainValidationOptions"][0][
        "ResourceRecord"
    ]
    ...
    if not click.confirm(
        f"Updating DNS record for cert {domain}\nDo you want to continue?"
    ):
        exit()
    ...
    wait_for_certificate_validation(
        client, certificate_arn=arn, sleep_time=5, timeout=600
    )
...
def create_hosted_zone(client, domain, start_domain, base_len):
    ...
    for x in reversed(
        range(len(domain_to_create) - (len(start_domain.split(".")) - 1))
    ):
        subdom = ".".join(domain_to_create[(x):]) + "."

        if not click.confirm(
            f"Do you wish to create domain {subdom}...\nDo you want to continue?"
        ):
            exit()
        ...
        nameserver_resource_records = [
            {"Value": nameserver} for nameserver in nameservers
        ]
...
@click.option(
    "--project-profile",
    help="aws account profile name for certificates account",
    required=True,
)
...
@click.option(
    "--project-profile",
    help="aws account profile name for application account",
    required=True,
)
    ...
    for listener in response["Listeners"]:
        if listener["Protocol"] == "HTTPS":
            ...
            response = acm_client.describe_certificate(
                CertificateArn=elb_client.describe_listener_certificates(
