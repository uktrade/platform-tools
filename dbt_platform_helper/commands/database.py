...
def copy(source_db: str, target_db: str):
    ...
    if not app or source_env or target_env:
    ...
    task_name = f"database-copy-{app.name}-{source_db}-{target_db}"
    ...
    if not addon_client_is_running(application, source_env, cluster_arn, task_name):
        subprocess.call(
            ...
            f"--env-vars SOURCE_DB_CONNECTION_SECRET='{source_db_connection}', TARGET_DB_CONNECTION_SECRET='{target_db_connection}'"
            "--platform-os linux "
            "--platform-arch arm64",
            shell=True,
        )
...
def _get_connection_string(session: Session, app: str, env: str, db_identifier: str) -> str:
    ...
    connection_string_parameter = (
        f"/copilot/{app}/{env}/secrets/{addon_name.upper()}_READ_ONLY_USER"
    )
    master_secret_name = f"/copilot/{app}/{env}/secrets/{addon_name.upper()}_RDS_MASTER_ARN"
