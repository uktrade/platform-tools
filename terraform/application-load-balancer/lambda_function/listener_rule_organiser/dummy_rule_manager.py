...
class DummyRuleManager:
    ...
    def rules(self):
        ...
        print('pulling list of current dummy rules from {}'.format(self.listener_arn))
        ...
        self._cached_rules = [r for r in current_rules.values() if
                              'reason' in r['Tags'] and r['Tags']['reason'] == 'dummy']

        return self._cached_rules

    def create_dummy_rule(self, target_group_arn, service_name):
        if service_name in [r['Tags']['service'] for r in self.rules]:
            print('service {} already has a dummy rule, exiting'.format(service_name))
            return
        ...
        print('creating dummy rule with priority {}'.format(next_priority))
    ...
    def delete_dummy_rule(self, service_name):
        ...
        if rule is None:
            print('service {} does not have a dummy rule, exiting'.format(service_name))
            return
