resource "aws_lb" "that" {
  ...
  name               = "${var.application}-${var.environment}-int"
  ...
  access_logs {
    ...
    prefix  = "${var.application}/${var.environment}/internal"
    enabled = true
  }
...
resource "aws_lb_listener" "alb-listener-internal-internal" {
...
#   for_each          = local.protocols
  load_balancer_arn = aws_lb.that.arn
  port              = local.protocols.http.port #each.value.port
  protocol          = "HTTP" #upper(each.key)
#   ssl_policy        = each.value.ssl_policy
#   certificate_arn   = each.value.certificate_arn
  tags              = local.tags
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.http-target-group-internal.arn
  }
}

resource "aws_security_group" "alb-security-group-internal" {
  ...
  for_each    = local.protocols
  name        = "${var.application}-${var.environment}-alb-${each.key}-internal"
...
resource "aws_lb_target_group" "http-target-group-internal" {
...
output "alb-arn-internal" {
  value = aws_lb.that.arn
}

resource "aws_cloudfront_vpc_origin" "alb-internal" {
  vpc_origin_endpoint_config {
    name                   = "${var.application}-${var.environment}-internal"
    arn                    = aws_lb.that.arn
    http_port              = local.protocols.http.port
    https_port             = local.protocols.https.port
...
# CloudFront is global - RAM share must be in us-east-1
resource "aws_ram_resource_share" "alb-internal" {
    name = "${var.application}-${var.environment}-internal"
    allow_external_principals = false
    provider = aws.ram-cdn
}

resource "aws_ram_principal_association" "alb-internal" {
  principal          = var.dns_account_id
  resource_share_arn = aws_ram_resource_share.alb-internal.arn
  provider = aws.ram-cdn
}

resource "aws_ram_resource_association" "alb-internal" {
  resource_arn       = aws_cloudfront_vpc_origin.alb-internal.arn
  resource_share_arn = aws_ram_resource_share.alb-internal.arn
  provider = aws.ram-cdn
}

output "cloudfront-vpc-origin-id" {
  value = aws_cloudfront_vpc_origin.alb-internal.id
}

output "cloudfront-vpc-origin-domain-name" {
    value = aws_lb.that.dns_name
}
...
#     resources = [aws_lb_listener.alb-listener-internal["https"].arn]
#   }
#   statement {
#     effect = "Allow"
#     actions = [
...
#       LISTENER_ARN = try(aws_lb_listener.alb-listener-internal["https"].arn, "")
#     }
#   }
