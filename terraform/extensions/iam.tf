...
data "aws_iam_policy_document" "ecs_service_access_for_codebase" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:elasticloadbalancing:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:listener/app/${var.args.application}-${var.environment}/*"
    ]
  }
...
data "aws_iam_policy_document" "validate_platform_config_for_codebase" {
  ...
  statement {
    ...
    resources = [
      ...
      "arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/platform/version/application/${var.args.application}/environment/${var.environment}/service/*",
    ]
  }
}
