...
data "aws_iam_policy_document" "assume_codepipeline_role" {
  statement {
    ...
    condition {
      ...
      values = [
        "arn:aws:codepipeline:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:${var.application}-${var.pipeline_name}-environment-pipeline"
      ]
    }
  }
}
...
data "aws_iam_policy_document" "assume_codebuild_role" {
  statement {
    ...
    condition {
      ...
      variable = "aws:SourceArn"
      values = compact([
        "arn:aws:codebuild:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:project/${var.application}-${var.pipeline_name}-environment-pipeline-plan",
        "arn:aws:codebuild:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:project/${var.application}-${var.pipeline_name}-environment-pipeline-build",
        "arn:aws:codebuild:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:project/${var.application}-${var.pipeline_name}-environment-pipeline-apply",
        local.triggers_another_pipeline ? "arn:aws:codebuild:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:project/${var.application}-${var.pipeline_name}-environment-pipeline-trigger" : null
      ])
    }
  }
...
data "aws_iam_policy_document" "origin_secret_rotate_access" {
  statement {
    ...
    resources = [
      for env in local.environment_config : "arn:aws:lambda:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:function:${var.application}-${env.name}-origin-secret-rotate"
    ]
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:wafv2:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:regional/managedruleset/*/*"
    ]
  }
  ...
  statement {
    ...
    resources = [
      for env in local.environment_config : "arn:aws:secretsmanager:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:secret:${var.application}-${env.name}-origin-verify-header-secret-*"
    ]
  }
...
data "aws_iam_policy_document" "assume_role_for_copilot_env_commands_policy_document" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:kms:${data.aws_region.current.id}:${local.triggering_account_id}:key/*"
    ]
  }
